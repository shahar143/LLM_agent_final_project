        ...
        if packet.haslayer(DNS) and packet.haslayer(IP):
            ...
            if dns.qr == 0 and dns.qdcount > 0:
                ...
                if response_packet:
                    ...
                    rdata_list = []
                    for ans in answers:
                        if ans.type == 15 or ans.type == 33 or ans.type==46:
...
def main():
    ...
    parser.add_argument("-p", "--path",  help="Path to the input PCAP file.")
    parser.add_argument("-o", "--output", default="dns_features_labeled.csv", help="CSV file to store the output data.")
    parser.add_argument("-l", "--label", default=0, help="the label to tag the data")
    parser.add_argument("-m", "--mode", choices=["overwrite", "append"], default="overwrite", help="Whether to overwrite or append to the output CSV file.")
    args = parser.parse_args()

    df = extract_features_from_pcap(args.path, int(args.label))
